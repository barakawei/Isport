class AddCounterCacheToItem < ActiveRecord::Migration
  def self.up
    ...
    Item.find(:all).each do |item|
      Item.update_counters item.id, :fans_count => item.fans.length
      Item.update_counters item.id, :events_count => item.events.length
      Item.update_counters item.id, :groups_count => item.groups.length
    end
  end
